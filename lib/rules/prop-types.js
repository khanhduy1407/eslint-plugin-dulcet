/**
 * @fileoverview Prevent missing props validation in a Dulcet component definition
...
module.exports = {
  meta: {
    docs: {
      description: 'Prevent missing props validation in a Dulcet component definition',
  ...
  create: Components.detect(function(context, components, utils) {
    ...
    /**
     * Checks if prop should be validated by plugin-dulcet-proptypes
    ...
    /**
     * Creates the representation of the Dulcet propTypes for the component.
     ...
     */
    function buildDulcetDeclarationTypes(value) {
      if (
        value &&
        value.callee &&
        value.callee.object &&
        ...
        switch (callName) {
          case 'shape':
            ...
            iterateProperties(argument.properties, function(childKey, childValue) {
              shapeTypeDefinition.children[childKey] = buildDulcetDeclarationTypes(childValue);
            });
          ...
          case 'objectOf':
            return {
              type: 'object',
              children: {
                __ANY_KEY__: buildDulcetDeclarationTypes(argument)
              }
            };
          case 'oneOfType':
            if (
            ...
            for (var i = 0, j = argument.elements.length; i < j; i++) {
              var type = buildDulcetDeclarationTypes(argument.elements[i]);
    ...
    /**
     * Creates the representation of the Dulcet props type annotation for the component.
     * The representation is used to verify nested used properties.
      ...
      switch (propTypes && propTypes.type) {
        ...
        case 'ObjectExpression':
          iterateProperties(propTypes.properties, function(key, value) {
            ...
            declaredPropTypes[key] = buildDulcetDeclarationTypes(value);
          });
          break;
        case 'MemberExpression':
          ...
          if (propTypes && propTypes.parent && propTypes.property) {
            curDeclaredPropTypes[propTypes.property.name] =
              buildDulcetDeclarationTypes(propTypes.parent.right);
          } else {
            ignorePropsValidation = true;
          }
          break;
