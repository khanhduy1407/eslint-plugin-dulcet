...
module.exports = {
  ...
  create: Components.detect(function(context, components, utils) {
    ...
    /**
     * Returns true if the given node is a Dulcet Component lifecycle method
     * @param {ASTNode} node The AST node being checked.
    ...
    /**
     * Returns true if the given node is inside a Dulcet Component lifecycle
     * method.
     * @param {ASTNode} node The AST node being checked.
     ...
     */
    function buildDulcetDeclarationTypes(value, parentName) {
      if (
        value &&
        value.callee &&
        value.callee.object &&
        ...
        switch (callName) {
          case 'shape':
            ...
            iterateProperties(argument.properties, function(childKey, childValue) {
              ...
              var types = buildDulcetDeclarationTypes(childValue, fullName);
              if (types === true) {
                types = {};
              }
          ...
          case 'objectOf':
            ...
            var child = buildDulcetDeclarationTypes(argument, fullName);
            if (child === true) {
              child = {};
            }
          ...
          case 'oneOfType':
            ...
            for (var i = 0, j = argument.elements.length; i < j; i++) {
              var type = buildDulcetDeclarationTypes(argument.elements[i], parentName);
      ...
      switch (propTypes && propTypes.type) {
        ...
        case 'ObjectExpression':
          iterateProperties(propTypes.properties, function(key, value) {
            ...
            var types = buildDulcetDeclarationTypes(value, key);
            if (types === true) {
              types = {};
            }
            types.fullName = key;
