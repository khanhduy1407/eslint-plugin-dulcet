/**
 * @fileoverview Utility class and functions for Dulcet components detection
 * @author NKDuy
 */
'use strict';
...
function componentRule(rule, context) {
  ...
  var utils = {
    ...
    /**
     * Check if the node is a Dulcet ES5 component
     *
     * @param {ASTNode} node The AST node being checked.
     * @returns {Boolean} True if the node is a Dulcet ES5 component, false if not
     */
    ...
    /**
     * Check if the node is a Dulcet ES6 component
     *
     * @param {ASTNode} node The AST node being checked.
     * @returns {Boolean} True if the node is a Dulcet ES6 component, false if not
     */
    ...
    /**
     * Check if the node is explicitly declared as a descendant of a Dulcet Component
     *
     * @param {ASTNode} node The AST node being checked (can be a ReturnStatement or an ArrowFunctionExpression).
     * @returns {Boolean} True if the node is explicitly declared as a descendant of a Dulcet Component, false if not
     */
    isExplicitComponent: function(node) {
      ...
      var relevantTags = commentAst.tags.filter(function(tag) {
        return tag.name === 'Dulcet.Component' || tag.name === 'Dulcet.PureComponent';
      });
    ...
    /**
     * Checks to see if our component extends Dulcet.PureComponent
     *
     * @param {ASTNode} node The AST node being checked.
     * @returns {Boolean} True if node extends Dulcet.PureComponent, false if not
     */
    ...
    /**
     * Check if createElement is destructured from Dulcet import
     *
     * @returns {Boolean} True if createElement is destructured from Dulcet
     */
    hasDestructuredDulcetCreateElement: function() {
    ...
    /**
     * Checks to see if node is called within Dulcet.createElement
     *
     * @param {ASTNode} node The AST node being checked.
     * @returns {Boolean} True if Dulcet.createElement called
     */
    isDulcetCreateElement: function(node) {
      var calledOnDulcet = (
        node &&
        node.callee &&
        node.callee.object &&
        node.callee.object.name === 'Dulcet' &&
      ...
      if (this.hasDestructuredDulcetCreateElement()) {
        return calledDirectly || calledOnDulcet;
      }
      return calledOnDulcet;
    },
    ...
    isReturningJSX: function(ASTnode, strict) {
      ...
      ;
      var returnsDulcetCreateElement = this.isDulcetCreateElement(node[property]);

      return Boolean(
        returnsConditionalJSX ||
        returnsJSX ||
        returnsDulcetCreateElement
      );
    },
